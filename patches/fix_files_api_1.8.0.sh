...
grep "GCB_PRODUCT_VERSION: '1.8.0'" app.yaml > /dev/null 2>&1

if [[ $? -eq 0 ]] ; then
  echo "Looks like a Course Builder 1.8.0 release; proceeding."; 
else 
  echo '------------------------- ERROR -----------------------------'
  echo "This patch will only apply cleanly against Course Builder "
  echo "version 1.8.0.  You may try manually applying the patch by "
...
cat > $TMPFILE <<EOF
diff -rupN coursebuilder/appengine_config.py coursebuilder-modified/appengine_config.py
--- coursebuilder/appengine_config.py	2015-04-02 19:31:58.000000000 -0700
+++ coursebuilder-modified/appengine_config.py	2015-06-05 17:02:46.704775531 -0700
@@ -63,17 +63,26 @@ class _Library(object):
...
@@ -108,7 +120,7 @@ def _third_party_libs_from_env():
...
diff -rupN coursebuilder/common/utils.py coursebuilder-modified/common/utils.py
--- coursebuilder/common/utils.py	2015-04-02 19:31:58.000000000 -0700
+++ coursebuilder-modified/common/utils.py	2015-06-05 16:58:57.838481631 -0700
...
Binary files coursebuilder/lib/appengine-mapreduce-0.8.2.zip and coursebuilder-modified/lib/appengine-mapreduce-0.8.2.zip differ
Binary files coursebuilder/lib/GoogleAppEngineCloudStorageClient-1.9.15.0.zip and coursebuilder-modified/lib/GoogleAppEngineCloudStorageClient-1.9.15.0.zip differ
Binary files coursebuilder/lib/GoogleAppEnginePipeline-1.9.17.0.zip and coursebuilder-modified/lib/GoogleAppEnginePipeline-1.9.17.0.zip differ
Binary files coursebuilder/lib/Graphy-1.0.0.zip and coursebuilder-modified/lib/Graphy-1.0.0.zip differ
Binary files coursebuilder/lib/simplejson-3.7.1.zip and coursebuilder-modified/lib/simplejson-3.7.1.zip differ
diff -rupN coursebuilder/models/jobs.py coursebuilder-modified/models/jobs.py
--- coursebuilder/models/jobs.py	2015-04-02 19:32:02.000000000 -0700
+++ coursebuilder-modified/models/jobs.py	2015-06-05 17:13:47.255352994 -0700
...
@@ -194,12 +196,13 @@ class StoreMapReduceResults(base_handler
...
@@ -221,6 +224,20 @@ class StoreMapReduceResults(base_handler
...
@@ -432,6 +449,19 @@ class MapReduceJob(DurableJobBase):
...
+
         kwargs = {
             ...
             'mapper_spec': '%s.%s.map' % (
@@ -441,9 +471,9 @@ class MapReduceJob(DurableJobBase):
             'input_reader_spec':
...
diff -rupN coursebuilder/modules/mapreduce/mapreduce_module.py coursebuilder-modified/modules/mapreduce/mapreduce_module.py
--- coursebuilder/modules/mapreduce/mapreduce_module.py	2015-04-02 19:32:02.000000000 -0700
+++ coursebuilder-modified/modules/mapreduce/mapreduce_module.py	2015-06-05 17:09:13.172627078 -0700
@@ -17,13 +17,14 @@
...
diff -rupN coursebuilder/scripts/common.sh coursebuilder-modified/scripts/common.sh
--- coursebuilder/scripts/common.sh	2015-04-02 19:32:04.000000000 -0700
+++ coursebuilder-modified/scripts/common.sh	2015-06-05 17:08:33.876236490 -0700
@@ -206,18 +206,21 @@ fi
...
diff -rupN coursebuilder/tests/functional/actions.py coursebuilder-modified/tests/functional/actions.py
--- coursebuilder/tests/functional/actions.py	2015-04-02 19:32:04.000000000 -0700
+++ coursebuilder-modified/tests/functional/actions.py	2015-06-05 16:59:01.734520780 -0700
@@ -120,6 +120,7 @@ class OverriddenEnvironment(object):
...
@@ -147,6 +148,7 @@ class OverriddenConfig(object):
...
@@ -155,6 +157,48 @@ class OverriddenConfig(object):
...
diff -rupN coursebuilder/tests/functional/model_analytics.py coursebuilder-modified/tests/functional/model_analytics.py
--- coursebuilder/tests/functional/model_analytics.py	2015-04-02 19:32:04.000000000 -0700
+++ coursebuilder-modified/tests/functional/model_analytics.py	2015-06-05 16:59:01.734520780 -0700
...
@@ -44,7 +46,6 @@ from modules.data_source_providers impor
...
@@ -642,22 +643,22 @@ class CronCleanupTest(actions.TestBase):
...
@@ -680,7 +681,8 @@ class CronCleanupTest(actions.TestBase):
...
@@ -741,12 +743,12 @@ class CronCleanupTest(actions.TestBase):
...
@@ -756,12 +758,12 @@ class CronCleanupTest(actions.TestBase):
...
+        self._assert_cloudstore_paths_removed(COURSE_ONE, paths)
     ...
     def test_cleanup_modifies_incomplete_status(self):
         mapper = rest_providers.LabelsOnStudentsGenerator(self.course_one)
@@ -793,19 +795,19 @@ class CronCleanupTest(actions.TestBase):
             self.execute_all_deferred_tasks()
...
+        self._assert_cloudstore_paths_removed(COURSE_TWO, course_two_paths)
     ...
     def test_cleanup_handler(self):
         mapper = rest_providers.LabelsOnStudentsGenerator(self.course_one)
@@ -820,7 +822,8 @@ class CronCleanupTest(actions.TestBase):
...
+                 headers={'X-AppEngine-Cron': 'True'})
         ...
         self.assertEquals(0, self._get_num_root_jobs(COURSE_ONE))
diff -rupN coursebuilder/tests/functional/module_config_test.py coursebuilder-modified/tests/functional/module_config_test.py
--- coursebuilder/tests/functional/module_config_test.py	2015-04-02 19:32:04.000000000 -0700
+++ coursebuilder-modified/tests/functional/module_config_test.py	2015-06-05 16:59:01.734520780 -0700
...
diff -rupN coursebuilder/tests/functional/modules_analytics.py coursebuilder-modified/tests/functional/modules_analytics.py
--- coursebuilder/tests/functional/modules_analytics.py	2015-04-02 19:32:04.000000000 -0700
+++ coursebuilder-modified/tests/functional/modules_analytics.py	2015-06-05 16:59:01.734520780 -0700
@@ -94,11 +94,13 @@ class AbstractModulesAnalyticsTest(actio
...
@@ -107,14 +109,16 @@ class AbstractModulesAnalyticsTest(actio
...
diff -rupN coursebuilder/tests/functional/test_classes.py coursebuilder-modified/tests/functional/test_classes.py
--- coursebuilder/tests/functional/test_classes.py	2015-04-02 19:32:04.000000000 -0700
+++ coursebuilder-modified/tests/functional/test_classes.py	2015-06-05 16:59:01.738520819 -0700
@@ -65,7 +65,8 @@ from tools import verify
...
diff -rupN coursebuilder/tests/suite.py coursebuilder-modified/tests/suite.py
--- coursebuilder/tests/suite.py	2015-04-02 19:32:04.000000000 -0700
+++ coursebuilder-modified/tests/suite.py	2015-06-05 16:59:01.738520819 -0700
@@ -212,6 +212,7 @@ class AppEngineTestBase(FunctionalTestBa
...
diff -rupN coursebuilder/tools/etl/etl.py coursebuilder-modified/tools/etl/etl.py
--- coursebuilder/tools/etl/etl.py	2015-04-02 19:32:04.000000000 -0700
+++ coursebuilder-modified/tools/etl/etl.py	2015-06-05 16:59:01.738520819 -0700
@@ -207,12 +207,14 @@ _DELETE_DATASTORE_CONFIRMATION_INPUT = '
